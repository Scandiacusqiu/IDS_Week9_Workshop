...
```{r}
weather_Portland01 <- weather_Portland %>% 
  mutate(RainToday = case_when(
    RainToday == "Yes" ~ 1,
    RainToday == "No" ~ 0
...
Raintomorrow_fit <- linear_reg() %>% 
  set_engine("lm") %>% 
  fit(RainTomorrow ~ RainToday, data = weather_Portland01)
tidy(Raintomorrow_fit)
```
...
```{r}
weather_split <- initial_split(weather_Portland, prop = 0.8)
...
weather_mod2 <- logistic_reg() %>% 
  set_engine("glm")
weather_wflow2 <- workflow() %>% # initiate workflow
  add_model(weather_mod2) %>%                   # add model
